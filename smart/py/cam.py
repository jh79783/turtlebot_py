...
def main() :
	...
	cam.cap_right = cv2.VideoCapture(1)
	cam.cap_left = cv2.VideoCapture(2)
	if ((cam.cap_right.isOpened() == False) and (cam.cap_left.isOpened() == False)): 
	...
	while not rospy.is_shutdown():
		...
		if (cam.ret_right == True)and (cam.ret_left == True): 
